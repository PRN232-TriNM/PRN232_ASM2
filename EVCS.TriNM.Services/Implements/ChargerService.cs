...
namespace EVCS.TriNM.Services.Implements
{
    public class ChargerService : IChargerService
    {
        ...
        {
            var result = await _unitOfWork.ChargerTriNMRepository.GetAllAsync();
        ...
        {
            var result = await _unitOfWork.ChargerTriNMRepository.GetByStationIdAsync(stationId);
            return result?.ToList() ?? new List<ChargerTriNm>();
        }
        ...
        public async Task<IEnumerable<ChargerTriNm>> GetAvailableChargersAsync()
        {
            ...
            var materialized = allChargers?.ToList() ?? new List<ChargerTriNm>();
            return materialized.Where(c => c.IsAvailable).ToList();
        }
